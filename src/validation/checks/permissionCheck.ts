...
export async function runPermissionCheck(
  ...
  try {
    ...
    const resp = await jiraClient.request<MyPermissionsResponse>({
    ...
    for (const permDef of PERMISSIONS_TO_CHECK) {
      ...
      if (!perm) {
        ...
        results.push({
          ...
          severity: 'medium',
          message:
            'Não foi possível determinar com segurança se a permissão está disponível via /mypermissions.',
          blocking: false,
        });
        continue;
      }
      ...
      if (perm.havePermission) {
        ...
        const isCritical =
          permDef.key === 'ADMINISTER_PROJECTS' ||
    ...
    results.push({
      ...
      severity: 'medium',
      message:
        'Não foi possível ler permissões via /mypermissions. Não é possível afirmar com certeza se permissões mínimas existem.',
