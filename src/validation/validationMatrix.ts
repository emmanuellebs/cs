...
import { loadFieldsProvisionConfig, FieldKind } from '../config/fieldsConfig';
...
import { ProvisioningReport } from '../utils/types';
...
function expectedCustomType(kind: FieldKind): string {
  switch (kind) {
    case 'number':
...
async function listAllFields(): Promise<JiraField[]> {
  const resp = await jiraClient.request<JiraField[]>({ url: '/field', method: 'GET' });
  return resp.data || [];
}

async function listContexts(fieldId: string): Promise<JiraFieldContext[]> {
  const resp = await jiraClient.request<{ values: JiraFieldContext[] }>({
...
export async function generateValidationMatrix(input: ValidationInput): Promise<void> {
  const logger = new Logger({ module: 'validationMatrix' });
  ...
  if (input.issueTypeId) {
    try {
      const cm = await jiraClient.request({
        method: 'get',
  ...
  if (input.sampleIssueKey) {
    try {
      const em = await jiraClient.request({
        method: 'get',
    ...
    try {
      const issueResp = await jiraClient.request({
        method: 'get',
  ...
  for (const cfg of fieldsCfg.fields) {
    ...
    const jiraField =
      (jiraIdFromMap && fieldsById.get(jiraIdFromMap)) || fieldsByName.get(cfg.name);
    const jiraId = jiraField?.id || jiraIdFromMap || '';
    const exists = Boolean(jiraField);
    const typeCompatible =
      exists && (jiraField!.schema?.custom || jiraField!.schema?.type) === expectedType;
    ...
    if (jiraId) {
      try {
        ...
        if (input.issueTypeId) {
          contextIncludesIssueType = contexts.some(
            (c) =>
              !c.issueTypeIds?.length ||
    ...
    const availableEdit =
      jiraId && Object.keys(editFields).length ? editFields.hasOwnProperty(jiraId) : availableCreate;
    const availableView = jiraId ? issueFields.hasOwnProperty(jiraId) || availableEdit : false;
    const filled =
      jiraId && issueFields[jiraId] !== undefined && issueFields[jiraId] !== null ? true : false;

    const usable = availableCreate || availableEdit || availableView || contextIncludesIssueType;

    const observations: string[] = [];
    if (!exists) observations.push('Campo não encontrado no Jira');
    if (exists && !typeCompatible) observations.push('Tipo incompatível');
    if (exists && !contextIncludesProject) observations.push('Contexto não inclui o projeto');
    if (!usable) observations.push('Campo não aparece no Cliente');
    if (availableCreate && !filled && input.sampleIssueKey)
      observations.push('Não populado na issue exemplo');

    md += `| ${cfg.name} | ${cfg.kind} | ${jiraId || '-'} | ${yesNo(exists)} | ${yesNo(
      typeCompatible
    )} | ${yesNo(contextIncludesProject)} | ${yesNo(usable)} | ${yesNo(
      availableCreate
    )} | ${yesNo(availableEdit || availableView)} | ${yesNo(filled)} | ${
      observations.join('; ') || ''
    } |\n`;
  }
  ...
  let projectIssueTypes: Array<{ id: string; name: string }> = [];
  try {
  ...
  for (const f of filtersCfg.filters) {
    ...
    const jql = result?.metadata && (result.metadata['jql'] as string);
    const expectedJql = f.jqlTemplate.replace(/{projectKey}/g, input.projectKey);
    const jqlOk = jql ? jql === expectedJql : false;
    md += `| ${f.name} | ${yesNo(exists)} | ${yesNo(jqlOk)} | ${
      exists ? '' : 'Filtro não encontrado ou falhou'
    } |\n`;
  }
  ...
  for (const d of dashboardsCfg.dashboards) {
    ...
    const exists = result ? result.status !== 'failed' && result.status !== 'manual' : false;
    md += `| ${d.name} | ${yesNo(exists)} | não automatizado | ${
      exists ? '' : 'Criar/validar gadgets manualmente'
    } |\n`;
  }
  ...
  md += '| --------- | ---------- | ------ | ------------------ |\n';
  md += `| ${sample?.jiraKey || '-'} | Cliente | ${sample?.status || '-'} | ${
    sample?.metadata?.['fieldsPopulated'] ?? ''
  } |\n`;

  writeTextFile('outputs/validation-matrix.md', md);
