...
import { ProvisioningReport, ProvisioningItemResult } from '../utils/types';
...
const logger = new Logger({ module: 'documentationService' });

function sectionFromItems(
  title: string,
  items: ProvisioningItemResult[],
  includeDetails = true
): string {
  if (!items.length) {
  ...
  if (includeDetails) {
    ...
    for (const i of items) {
      md += `| ${i.key} | ${i.name ?? ''} | ${i.status} | ${
        i.details ? i.details.replace(/\|/g, '\\|') : ''
      } |\n`;
    }
    md += '\n';
  }
...
export function generateManualSteps(report: ProvisioningReport): void {
  const manualOrFailed: ProvisioningItemResult[] = [];
  const add = (arr: ProvisioningItemResult[]) =>
    manualOrFailed.push(...arr.filter((i) => i.status === 'manual' || i.status === 'failed'));
  ...
  writeTextFile('docs/MANUAL_STEPS.md', md);
  logger.info('docs/MANUAL_STEPS.md gerado.');
}

export function generateAutomationBlueprint(): void {
...
- **Objetivo**: Identificar oportunidades de expansão a partir de alto engajamento.\n- **Gatilho**: Issue updated ou agendado.\n- **Condição**:\n  - Engajamento da plataforma = Alto **e/ou** Health Score acima de limiar (ex.: > 80).\n- **Ação**:\n  - Criar issue do tipo **Oportunidade CS**.\n- **Tipo de issue gerado**: Oportunidade CS.\n- **Observações**: Pode incluir valor estimado e próxima etapa sugerida.\n`;
  ...
  writeTextFile('docs/AUTOMATION_BLUEPRINT.md', md);
  logger.info('docs/AUTOMATION_BLUEPRINT.md gerado.');
}

export function generateFieldMapping(report: ProvisioningReport): void {
  ...
  writeTextFile('docs/FIELD_MAPPING.md', md);
  logger.info('docs/FIELD_MAPPING.md gerado.');
}

export function generateJqlFilters(report: ProvisioningReport): void {
  const filtersConfig = loadFiltersProvisionConfig();
  ...
  for (const cfg of filtersConfig.filters) {
    ...
    const jql =
      (result?.metadata && (result.metadata['jql'] as string)) ||
      cfg.jqlTemplate.replace(/{projectKey}/g, loadProjectProvisionConfig().jira.projectKey);
  ...
  writeTextFile('docs/JQL_FILTERS.md', md);
  logger.info('docs/JQL_FILTERS.md gerado.');
}

export function generateDashboardSetup(report: ProvisioningReport): void {
  const dashboardsConfig = loadDashboardsProvisionConfig();
  ...
  writeTextFile('docs/DASHBOARD_SETUP.md', md);
  logger.info('docs/DASHBOARD_SETUP.md gerado.');
}

export function generateProjectArchitecture(): void {
...
- **Nenhuma falha isolada** interrompe o provisionamento completo; cada item é marcado com \`created\`, \`reused\`, \`skipped\`, \`manual\` ou \`failed\`.\n- **Workflows e colunas de board** não são configurados por API, apenas documentados.\n- **Dashboards e gadgets** são parcialmente automatizados (criação de dashboards) e complementados com blueprint manual.\n`;
  ...
  writeTextFile('docs/PROJECT_ARCHITECTURE.md', md);
  logger.info('docs/PROJECT_ARCHITECTURE.md gerado.');
}

export function generateAllDocumentation(report: ProvisioningReport): void {
