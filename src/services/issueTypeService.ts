...
import {
  IssueTypeConfig,
  IssueTypesProvisionConfig,
  LogicalIssueTypeKey,
  loadIssueTypesProvisionConfig,
} from '../config/issueTypesConfig';

const logger = new Logger({ module: 'issueTypeService' });
...
export interface IssueTypesProvisionResult {
  results: ProvisioningItemResult[];
  logicalToJiraTypeId: Record<LogicalIssueTypeKey, string | null>;
}

async function listAllIssueTypes(): Promise<JiraIssueType[]> {
  const resp = await jiraClient.request<JiraIssueType[]>({
    url: '/issuetype',
    method: 'GET',
  });
  return resp.data;
...
async function createIssueType(cfg: IssueTypeConfig): Promise<JiraIssueType> {
  const resp = await jiraClient.request<JiraIssueType>({
    url: '/issuetype',
    method: 'POST',
    data: {
...
export async function ensureIssueTypes(): Promise<IssueTypesProvisionResult> {
  const config: IssueTypesProvisionConfig = loadIssueTypesProvisionConfig();
  const allTypes = await listAllIssueTypes();

  const results: ProvisioningItemResult[] = [];
  const logicalToJiraTypeId: Record<LogicalIssueTypeKey, string | null> = {
    account: null,
    interaction: null,
    successPlan: null,
    risk: null,
  ...
  for (const cfg of config.issueTypes) {
    try {
      ...
      if (mode.mode === 'audit' || mode.dryRun) {
        logger.info(
          `Issue type ${cfg.name} não existe. Modo atual impede criação automática (mode=${mode.mode}, dryRun=${mode.dryRun}).`
        );
        ...
        if (fallback) {
          ...
          results.push(
            buildResult(cfg, 'reused', {
              jiraId: fallback.id,
              details:
                'Issue type customizado não criado em modo de auditoria/dry-run. Fallback para tipo padrão existente.',
              metadata: {
          ...
          results.push(
            buildResult(cfg, 'manual', {
              details:
                'Issue type customizado não existe e não pôde ser criado em modo atual. Nenhum fallback padrão encontrado. Criar manualmente.',
            })
          );
        }
        continue;
      ...
      try {
        ...
        logger.error(
          `Falha ao criar issue type ${cfg.name}. Tentando fallback para tipo padrão ${cfg.fallbackStandardTypeName}.`,
          err
        );
        ...
        if (fallback) {
          ...
          results.push(
            buildResult(cfg, 'reused', {
              jiraId: fallback.id,
              details:
                'Falha ao criar issue type customizado. Fallback para issue type padrão existente.',
              warnings: [
          ...
          results.push(
            buildResult(cfg, 'manual', {
              details:
                'Falha ao criar issue type customizado e nenhum fallback padrão foi encontrado. Necessário criar/ajustar manualmente.',
              error: err?.message,
            })
          );
        }
