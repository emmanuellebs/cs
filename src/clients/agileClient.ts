...
const logger = new Logger({ module: 'agileClient' });
...
export class AgileClient {
  ...
  async request<T = any>(
    ...
    try {
      ...
      logger.error(
        `Erro ao chamar Jira Agile API (${method.toUpperCase()} ${config.url}) - status: ${status}`,
        data || error
      );
      throw error;
    }
...
export const agileClient = new AgileClient();
