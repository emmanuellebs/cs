...
exports.listScreenTabs = listScreenTabs;
exports.getScreenFields = getScreenFields;
exports.addFieldToScreen = addFieldToScreen;
exports.getIssueTypeScreenScheme = getIssueTypeScreenScheme;
exports.getScreensForIssueType = getScreensForIssueType;
exports.associateFieldToIssueTypeScreen = associateFieldToIssueTypeScreen;
exports.associateFieldsToIssueTypeScreens = associateFieldsToIssueTypeScreens;
const jiraClient_1 = require("../clients/jiraClient");
const logger_1 = require("../utils/logger");
const logger = new logger_1.Logger({ module: 'screenService' });
/**
...
async function listScreenTabs(screenId) {
    try {
        const response = await jiraClient_1.jiraClient.request({
            method: 'get',
...
async function createScreenTab(screenId, name) {
    try {
        const response = await jiraClient_1.jiraClient.request({
            method: 'post',
            url: `/screens/${screenId}/tabs`,
...
async function getFieldsForTab(screenId, tabId) {
    try {
        const response = await jiraClient_1.jiraClient.request({
            method: 'get',
 ...
 */
async function getScreenFields(screenId) {
    try {
 ...
 */
async function getIssueTypeScreenScheme(projectId) {
    try {
...
async function getScreenSchemeDetails(screenSchemeId) {
    try {
        const response = await jiraClient_1.jiraClient.request({
            method: 'get',
 ...
 */
async function getScreensForIssueType(projectId, issueTypeId) {
    try {
        logger.debug(`[SCREEN] Getting screens for issue type ${issueTypeId} in project ${projectId}...`);
        ...
        const relevantMappings = allMappings.filter((m) => String(m.issueTypeScreenSchemeId) === String(scheme.id) &&
        ...
        const mapping = relevantMappings.find((m) => m.issueTypeId === issueTypeId) || relevantMappings[0];
        ...
        logger.warn('[SCREEN] Could not resolve screens from scheme details; falling back to all screens');
        return catalog;
    }
    catch (err) {
 ...
 */
async function associateFieldToIssueTypeScreen(projectId, issueTypeId, fieldId, fieldName) {
    try {
        ...
        for (const screen of screens) {
            try {
                ...
                if (added) {
                    logger.info(`[LAYOUT] Campo ${fieldName} adicionado à tela ${screen.name} (${screen.id})`);
                    lastSuccess = true;
                    lastScreenId = String(screen.id);
                }
            }
 ...
 */
async function associateFieldsToIssueTypeScreens(projectId, issueTypeId, fieldsToAdd) {
    const results = [];
    logger.info(`[SCREEN] Batch associating ${fieldsToAdd.length} fields to issue type ${issueTypeId}...`);
    for (const field of fieldsToAdd) {
        const result = await associateFieldToIssueTypeScreen(projectId, issueTypeId, field.id, field.name);
        results.push({
            fieldId: field.id,
