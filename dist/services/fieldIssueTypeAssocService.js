...
exports.ensureFieldIssueTypeAssociations = ensureFieldIssueTypeAssociations;
exports.validateFieldsAvailableForAccount = validateFieldsAvailableForAccount;
...
const screenService_1 = require("./screenService");
const logger = new logger_1.Logger({ module: 'fieldIssueTypeAssocService' });
/**
 * Para cada issue type, associa todos os campos mapeados
 * Retorna um relatório de quais associações foram bem-sucedidas
 */
async function ensureFieldIssueTypeAssociations(projectId, issueTypeMap, fieldMap) {
    const results = [];
    ...
    const issueTypesConfig = (0, issueTypesConfig_1.loadIssueTypesProvisionConfig)();
    // Para cada issue type
    for (const issueTypeCfg of issueTypesConfig.issueTypes) {
        const issueTypeId = issueTypeMap[issueTypeCfg.key];
        if (!issueTypeId) {
            logger.warn(`[FIELD-ASSOC] Issue type "${issueTypeCfg.key}" not in map (likely not created). Skipping field associations.`);
            results.push({
        ...
        if (fieldsForThisType.length === 0) {
            logger.debug(`[FIELD-ASSOC] No fields configured for issue type "${issueTypeCfg.key}"`);
            continue;
        }
        logger.info(`[LINK] Associando ${fieldsForThisType.length} campos ao issue type "${issueTypeCfg.name}" (${issueTypeId})...`);
        ...
        if (jiraFields.length === 0) {
            logger.warn(`[FIELD-ASSOC] No field IDs found for issue type "${issueTypeCfg.key}"`);
            results.push({
        ...
        try {
            const assocResults = await (0, screenService_1.associateFieldsToIssueTypeScreens)(projectId, issueTypeId, jiraFields.map(f => ({ id: f.id, name: f.name })));
            const successCount = assocResults.filter(r => r.success).length;
            if (successCount === 0) {
                logger.warn(`[FIELD-ASSOC] No fields were successfully associated to issue type "${issueTypeCfg.key}"`);
                results.push({
                ...
                logger.warn(`[FIELD-ASSOC] Partial success: ${successCount}/${jiraFields.length} fields associated to issue type "${issueTypeCfg.key}"`);
                results.push({
                ...
                logger.info(`[LAYOUT] ✓ Todos os ${successCount} campos associados ao issue type "${issueTypeCfg.name}"`);
                results.push({
                    key: `field-assoc:${issueTypeCfg.key}`,
                    status: 'created',
            ...
            logger.error(`[FIELD-ASSOC] Error associating fields to issue type "${issueTypeCfg.key}":`, err);
            results.push({
    ...
    logger.info(`[FIELD-ASSOC] Field-to-issue-type associations complete. ${results.length} issue types processed.`);
    return { results };
}
/**
 ...
 * - etc.
 */
function getFieldsForIssueType(fieldsConfig, issueTypeKey) {
    ...
    if (issueTypeKey === 'account') {
        return fieldsConfig.fields.filter((f) => f.group === 'account' || extraGroupsForAccount.includes(f.group));
    }
 ...
 */
async function validateFieldsAvailableForAccount(projectId, issueTypeId, fieldIds) {
    ...
    for (const fieldId of fieldIds) {
        try {
            ...
            const testResult = await jiraClient_1.jiraClient.request({
                method: 'post',
                url: '/issues/picker',
                data: {
            ...
            if (testResult.status < 400) {
                ...
                logger.debug(`[FIELD-ASSOC] ✗ Field ${fieldId} validation failed (status: ${testResult.status})`);
            }
        }
        catch (err) {
            missingFields.push(fieldId);
    ...
    logger.info(`[FIELD-ASSOC] Validation: ${availableFields.length} available, ${missingFields.length} missing`);
    return {
        allAvailable,
        availableFields,
        missingFields,
